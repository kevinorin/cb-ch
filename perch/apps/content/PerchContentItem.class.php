                    ...
                    if (strpos($contentHistory, $web_path)===false) {
                        ...
                        foreach($resfiles as $resfile) {
                            if ($resfile == $bare.'-thumb'.'.'.$extension) {
                                $to_be_deleted[] = PERCH_RESFILEPATH . '/' . $resfile;
                            }
                        }
                        
                        // resized images
                        $pattern = '/'.str_replace('/', '\/', $bare).'-(w[0-9]{1,4}){0,1}(h[0-9]{1,4}){0,1}\.'.$extension.'/';
    ...
    public function pre_process_map($id, $tag, $value)
    {
        ...
        if (isset($value['adr'])) {
            ...
            if (!isset($value['lat'])) {
                ...
                $result = PerchUtil::http_get_request('https://', 'maps.googleapis.com', $path);
                if ($result) {
            ...
            $r  = '<img id="cmsmap'.PerchUtil::html($id).'" src="https://maps.google.com/maps/api/staticmap';
